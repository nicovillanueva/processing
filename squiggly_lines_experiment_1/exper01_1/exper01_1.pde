...
ArrayList<Integer> rotDirections = new ArrayList<Integer>();
...
void setup() {
  ...
  for(int i = 0; i < elements; i++) {
    ...
    colors.add(new Integer[]{Math.round(random(50,255)),Math.round(random(50,255)),Math.round(random(50,255))});
    sizes.add((((float)Math.random() * 2.5f) +2));
    directions.add(1);
    rotDirections.add(1);
  }
  bg = new Integer[]{Math.round(random(50,250)),Math.round(random(100,255)),Math.round(random(50,255))};
  noStroke();
}
...
void draw() {
  ...
  if(inc > TWO_PI) {
   ...
   // rotDir *= -1;
    ...
    for(int i = 0; i < elements; i++){
      ...
      rotDirections.set(i, Integer.valueOf(rotDirections.get(i) * -1));
    }
  }
  ...
  for(int i = 0; i < elements; i++){
    ...
    // Effect 2
    ellipse(item.x + (cos(inc) * 20) * directions.get(i), (item.y + cos(inc * rotDirections.get(i)) * 35), sizes.get(i), sizes.get(i));
  }
}
